...
if(input){input.addEventListener('change',()=>{const f=input.files&&input.files[0];if(!f)return;statusEl.textContent='Processando reconhecimento…';const reader=new FileReader();reader.onload=()=>{const img=new Image();img.onload=()=>{results.innerHTML='';const tiles=10;for(let i=0;i<tiles;i++){results.appendChild(makeTile(img))}statusEl.textContent='Encontramos '+tiles+' fotos do participante'};img.src=reader.result};reader.readAsDataURL(f)})}
