...
async function callWebhook(payload){const url=(webhookEl&&webhookEl.value)||'';if(!url){throw new Error('Informe a URL do webhook');}const res=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});if(!res.ok){throw new Error('Webhook retornou status '+res.status);}return await res.json().catch(()=>({}))}
...
async function processFile(file){statusEl.textContent='Processando…';const reader=new FileReader();reader.onload=async()=>{try{const dataUrl=reader.result;const img=new Image();img.onload=()=>{if(!webhookEl||!webhookEl.value){results.innerHTML='';results.appendChild(makeTile(img));statusEl.textContent='Pré-visualização local'} };img.src=dataUrl;const mode=(modeSel&&modeSel.value)||'recognition';if(webhookEl&&webhookEl.value){const payload={mode,watermark_text:wmTextEl?wmTextEl.value:'',image_base64:dataUrl};const json=await callWebhook(payload);renderWebhookResponse(json);statusEl.textContent='Resultado do webhook aplicado'} }catch(err){statusEl.textContent='Erro: '+err.message}};reader.readAsDataURL(file)}
